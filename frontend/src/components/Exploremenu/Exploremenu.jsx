...
const Exploremenu = ({category,setCatogory}) => {
  ...
  return (
    <div className='explore-menu relative flex flex-col gap-5 w-full sm:w-3/4  mx-auto' id='explore-menu'>
        ...
        <div className='exploremenu-list flex justify-between items-center sm:gap-8 text-center overflow-x-scroll transition-[0.2s]'>
            {
                menu_list.map((item,index)=>{
                    return(
                        <div key={index} className='cursor-pointer' onClick={() => setCatogory(item.menu_name)}>
                            <img src={item.menu_image} alt="" className={category === item.menu_name ? "border-4 border-orange-600 rounded-full" : ""}/>
                            <p>{item.menu_name}</p>
                        </div>
